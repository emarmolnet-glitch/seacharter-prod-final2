...
const [serviceSource, componentSource, entrySource, indexSource, envExample] = await Promise.all([
...
test('Comtrade service uses query authentication and seven-day browser cache', () => {
  assert.match(envExample, /^VITE_UN_COMTRADE_API_KEY=/m);
  assert.match(serviceSource, /https:\/\/comtradeapi\.un\.org\/data\/v1\/get\/C\/A\/HS/);
  assert.match(serviceSource, /searchParams\.set\('subscription-key', apiKey\)/);
  assert.doesNotMatch(serviceSource, /Ocp-Apim-Subscription-Key/);
  assert.match(serviceSource, /console\.error\('\[UN Comtrade\] Error de conexión o autorización\.'/);
