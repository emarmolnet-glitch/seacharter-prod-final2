...
test('chat header relies exclusively on the main assistant toggle for closing', () => {
  assert.doesNotMatch(frontendSource, /sca-close|icons\.close|Cerrar asistente/);
  assert.doesNotMatch(frontendSource, /event\.key === "Escape"/);
  assert.doesNotMatch(stylesheet, /\.sca-close/);
});
