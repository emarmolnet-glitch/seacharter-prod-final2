...
export function ForwarderWorkspace() {
  ...
  const refreshToastTimeoutRef = useRef(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (refreshToastTimeoutRef.current) clearTimeout(refreshToastTimeoutRef.current);
    };
  }, []);
  ...
  const getProjectDetails = async (projectRefOrId) => {
    try {
      ...
      if (!res.ok) throw new Error('No se pudo obtener el detalle del proyecto');
  ...
  const fetchProjects = async (options = {}) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('No se pudo sincronizar la lista de proyectos. Verifica la conexión.');
      }
      const data = await res.json();
      ...
      if (!silent) {
        if (projectsRef.current && projectsRef.current.length > 0) {
          setRefreshToast(friendlyMessage);
          if (refreshToastTimeoutRef.current) clearTimeout(refreshToastTimeoutRef.current);
          refreshToastTimeoutRef.current = setTimeout(() => {
            setRefreshToast(null);
          }, 6000);
          setError(null);
        } else {
      ...
      <div className={`w-full h-full flex overflow-hidden bg-slate-950 text-slate-100 font-sans relative ${showExecutiveReport ? 'print:hidden' : ''}`}>
        <aside className="w-80 shrink-0 bg-slate-900 border-r border-slate-800 flex flex-col h-full overflow-hidden print:hidden">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
            {/* Toast no intrusivo ante error de actualización conservando datos cacheados */}
            {refreshToast && (
              <div
                role="alert"
                className="p-3 bg-amber-950/80 border border-amber-500/40 rounded-xl text-xs text-amber-200 flex items-start justify-between gap-2.5 shadow-lg animate-fade-in"
              >
                <div className="flex items-start gap-2 min-w-0">
                  <div className="w-5 h-5 rounded-full bg-amber-500/20 flex items-center justify-center shrink-0 mt-0.5 text-amber-400">
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-amber-300 text-[11px]">Aviso de sincronización</p>
                    <p className="text-[11px] text-amber-200/90 leading-snug mt-0.5">{refreshToast}</p>
                    <p className="text-[10px] text-amber-300/70 mt-1 italic">Mostrando datos cacheados.</p>
                  </div>
                </div>
                <div className="flex items-center gap-1 shrink-0 pt-0.5">
                  <button
                    ...
                    disabled={isRefreshing}
                    className="text-[11px] font-semibold text-sky-400 hover:text-sky-300 underline cursor-pointer px-1 py-0.5 transition-colors disabled:opacity-50"
                  >
                    Reintentar
                  </button>
                  <button
                    ...
                    className="text-amber-400/80 hover:text-amber-100 p-0.5 rounded cursor-pointer transition-colors"
            ...
            {/* Feedback de carga con Skeleton Loading y Spinner elegante */}
            {isLoading && (
              projects.length === 0 ? (
                <div className="space-y-3 p-1 animate-pulse" role="status" aria-label="Cargando proyectos">
                  <div className="flex items-center justify-center gap-2 py-2 text-xs text-sky-400">
                    <div className="inline-block animate-spin w-4 h-4 border-2 border-sky-400 border-t-transparent rounded-full"></div>
                    <p>Cargando proyectos...</p>
                  </div>
                  {[1, 2, 3, 4].map((n) => (
                    <div key={n} className="p-3.5 rounded-xl border border-slate-800 bg-slate-900/50 space-y-2.5">
                      <div className="flex justify-between items-center">
                        <div className="h-3 bg-slate-700/60 rounded w-24"></div>
                        <div className="h-4 w-4 bg-slate-800/80 rounded"></div>
                      </div>
                      <div className="h-4 bg-slate-700/80 rounded w-3/4"></div>
                      <div className="flex gap-2 pt-1">
                        <div className="h-2.5 bg-slate-800/80 rounded w-16"></div>
                ...
                <div className="px-3 py-2 flex items-center justify-between text-xs text-sky-400 bg-sky-950/40 border border-sky-800/50 rounded-lg animate-pulse">
                  <div className="flex items-center gap-2">
                    <div className="inline-block animate-spin w-3.5 h-3.5 border-2 border-sky-400 border-t-transparent rounded-full shrink-0"></div>
            ...
            {/* Manejo elegante de errores: Empty State amigable sin error HTTP en crudo */}
            {!isLoading && error && (
              projects.length === 0 ? (
                <div className="p-5 text-center rounded-xl bg-slate-900/70 border border-slate-800/80 shadow-inner flex flex-col items-center justify-center space-y-3 my-2">
                  <div className="w-12 h-12 rounded-full bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                  </div>
                  <div className="space-y-1 px-2">
                    <h4 className="text-sm font-semibold text-slate-200">Sincronización pendiente</h4>
                  ...
                  <button
                    ...
                    className="mt-1 px-4 py-2 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white font-medium text-xs rounded-lg shadow-sm cursor-pointer transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                    ...
                    <span>{isLoading || isRefreshing ? 'Reintentando...' : 'Reintentar'}</span>
                  </button>
                </div>
              ) : null
            )}
